...
def index(request, blogname):
    bl = Blog.objects.get(urlname=blogname) #TODO: handle DoesNotExist 
    ...
    return render(request, 'news/index.html', {'blog': bl.urlname,
...
def detail(request, blogname, year, month, day, slug):
    bl = Blog.objects.get(urlname=blogname)
    try:
    ...
    return render(request, 'news/detail.html', {'blog': bl.urlname, 'entry': e, 'prev_entry': prev_entry, 'next_entry': next_entry})
...
def day_archive(request, blogname, year, month, day):
    ...
    pd = cd - one_day
    pd = pd if Entry.public.exclude(blog=bl,
                                    created_at__year=cd.year, 
                                    created_at__month=cd.month, 
                                    created_at__day=cd.day).filter(
                                        created_at__lt=cd).exists() else None
    nd = cd + one_day
    nd = nd if Entry.public.exclude(blog=bl,
                                    created_at__year=cd.year, 
                                    created_at__month=cd.month, 
                                    created_at__day=cd.day).filter(
...
def year_archive(request, blogname, year):
    ...
    return render(request, 'news/year_archive.html', {'blog': bl.urlname, 
                                                      'months': months,
                                                      'prev_year': py,
                                                      'next_year': ny,
                                                      'current_year': cy
